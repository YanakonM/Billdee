# ทำ Billdee เป็นแอปติดตั้ง (.exe) เก็บข้อมูลใน SQLite
...
> ✅ ฝั่งโค้ดรองรับ Tauri แล้ว — ใช้คำสั่งด้านล่างเพื่อทดสอบหรือ build เป็นตัวติดตั้ง Windows
...
ครั้งแรกจะคอมไพล์ Rust นานหน่อย (หลายนาที) แล้วจะเด้งหน้าต่างแอป Billdee ขึ้นมา
...
ได้ไฟล์ติดตั้งที่:
```
src-tauri\target\release\bundle\nsis\Billdee_0.1.17_x64-setup.exe   ← ตัวติดตั้ง (แนะนำ)
src-tauri\target\release\bundle\msi\Billdee_0.1.17_x64_en-US.msi
```
...
2. เปิดแอป Billdee ที่ติดตั้ง → ตั้งค่า → สำรอง/กู้คืน → **นำเข้า (Import)** เลือกไฟล์ JSON นั้น
...
- ถ้าเปิดแล้วหน้าต่าง**ขาวเปล่า** หลัง build: ตรวจ `vite.config.js` ว่ามี `base: './'` หรือแจ้ง error ที่ build/เปิดแอปให้ตรวจต่อ
  แล้ว build ใหม่
